...
#ifdef GET_MIPS
    mic_context_t *gc = mic_get_context();
    unsigned long long destArray_circle1 = get_cpu_circle();
#endif
#ifndef USE_SIMD
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
#ifdef VERIFY_SIMD
    float *myFFTworkingArrayRealTmp = newTable1D(myFFT->FFT_SIZE,sizeof(float));
    float *myFFTworkingArrayImagTmp = newTable1D(myFFT->FFT_SIZE,sizeof(float));
    float *destArrayRealTmp = newTable1D(myFFT->FFT_SIZE,sizeof(float));
    float *destArrayImagTmp = newTable1D(myFFT->FFT_SIZE,sizeof(float));
    ...
    float valueBReal_;
    float valueBImag_;
    float newValueAReal_;
    float newValueAImag_;
    float newValueBReal_;
    float newValueBImag_;

    unsigned int accumulatorA_;
    unsigned int accumulatorR_;

    unsigned int numberGroups_;
    unsigned int numberSubGroups_;

    unsigned int indexLevel_;
    unsigned int indexGroup_;
    unsigned int indexSubGroup_;

    unsigned int indexRevBitOrder_;
    ...
    numberGroups_ = 1;
    numberSubGroups_ = myFFT->FFT_HALFSIZE;

    for (indexArray_ = 0; indexArray_ < myFFT->FFT_SIZE; indexArray_++)
    {
        myFFTworkingArrayRealTmp[indexArray_] = sourceArrayReal[indexArray_];
        myFFTworkingArrayImagTmp[indexArray_] = sourceArrayImag[indexArray_];
    }

    for (indexLevel_ = 0; indexLevel_ < myFFT->FFT_NBLEVELS; indexLevel_++)
    {
        accumulatorA_ = 0;
        accumulatorR_ = 0;

        for (indexGroup_ = 0; indexGroup_ < numberGroups_; indexGroup_++)
        {
            for (indexSubGroup_ = 0; indexSubGroup_ < numberSubGroups_; indexSubGroup_ ++)
            {
                a_ = accumulatorA_;
                b_ = accumulatorA_ + numberSubGroups_;
                r_ = accumulatorR_;
                accumulatorA_ ++;
                accumulatorR_ += numberGroups_;

                valueAReal_ = myFFTworkingArrayRealTmp[a_];
                valueAImag_ = myFFTworkingArrayImagTmp[a_];
                valueBReal_ = myFFTworkingArrayRealTmp[b_];
                valueBImag_ = myFFTworkingArrayImagTmp[b_];

                newValueAReal_ = valueAReal_ + valueBReal_;
                newValueAImag_ = valueAImag_ + valueBImag_;

                diffABReal_ = valueAReal_ - valueBReal_;
                diffABImag_ = valueAImag_ - valueBImag_;
                curWnReal_ = myFFT->WnReal[r_];
                curWnImag_ = myFFT->WnImag[r_];
                newValueBReal_ = diffABReal_ * curWnReal_ - diffABImag_ * curWnImag_;
                newValueBImag_ = diffABReal_ * curWnImag_ + diffABImag_ * curWnReal_;

                myFFTworkingArrayRealTmp[a_] = newValueAReal_;
                myFFTworkingArrayImagTmp[a_] = newValueAImag_;
                myFFTworkingArrayRealTmp[b_] = newValueBReal_;
                myFFTworkingArrayImagTmp[b_] = newValueBImag_;
            }

            accumulatorA_ += numberSubGroups_;
            accumulatorR_ = 0;
        }
    ...
    for (indexRevBitOrder_ = 0; indexRevBitOrder_ < myFFT->FFT_SIZE; indexRevBitOrder_++)
    {
        destArrayRealTmp[indexRevBitOrder_] = myFFTworkingArrayRealTmp[myFFT->revBitOrderArray[indexRevBitOrder_]];
        destArrayImagTmp[indexRevBitOrder_] = myFFTworkingArrayImagTmp[myFFT->revBitOrderArray[indexRevBitOrder_]];
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    {
        float max_real;
        if ( fabs(destArrayReal[indexRevBitOrder]) >= fabs(destArrayRealTmp[indexRevBitOrder]) )
        {
            max_real = fabs(destArrayReal[indexRevBitOrder]);
        }
        else
        {
            max_real=fabs(destArrayRealTmp[indexRevBitOrder]);
        }

        if ( (fabs(destArrayReal[indexRevBitOrder] - destArrayRealTmp[indexRevBitOrder])/(max_real + 1E-8 ) > REL_TOL) && (fabs(destArrayReal[indexRevBitOrder] - destArrayRealTmp[indexRevBitOrder]) > ABS_TOL))
        {
           log_error("this is real compare  SIMD  K=%d\n",indexRevBitOrder );
           log_error("#FFT Real# : %f; SIMD: %f\n ", destArrayRealTmp[indexRevBitOrder], destArrayReal[indexRevBitOrder]);
           exit(-1);
        }


        float max_image;
        if ( fabs(destArrayImag[indexRevBitOrder]) >= fabs(destArrayImagTmp[indexRevBitOrder]) )
        {
            max_image = fabs(destArrayImag[indexRevBitOrder]);
        }
        else
        {
            max_image = fabs(destArrayImagTmp[indexRevBitOrder]);
        }
        if ( (fabs(destArrayImag[indexRevBitOrder]-destArrayImagTmp[indexRevBitOrder])/(max_image + 1E-8 ) > REL_TOL) && (fabs(destArrayImag[indexRevBitOrder] - destArrayImagTmp[indexRevBitOrder]) > ABS_TOL))
        {
           log_error("this is Image compare  SIMD  K=%d\n", indexRevBitOrder);
           log_error("#FFT Imag# : %f; SIMD: %f\n ", destArrayImagTmp[indexRevBitOrder], destArrayImag[indexRevBitOrder]);
           exit(-1);
        }
    }

    deleteTable1D((void*)myFFTworkingArrayRealTmp);
    deleteTable1D((void*)myFFTworkingArrayImagTmp);
    deleteTable1D((void*)destArrayRealTmp);
    deleteTable1D((void*)destArrayImagTmp);
...
#ifdef GET_MIPS
    unsigned long long destArray_circle2 = get_cpu_circle();
    gc->destArray_circle_sum += (destArray_circle2 - destArray_circle1);
#endif
...
#ifdef GET_MIPS
    mic_context_t *gc = mic_get_context();
    unsigned long long destArray2_circle1 = get_cpu_circle();
#endif
#ifndef USE_SIMD
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    for (k_ = 0; k_ < myFFT->FFT_SIZE; k_++)
    {
        destArrayImagTmp[k_] = -1.0f * sourceArrayImag[k_];
    }

    fftCompute(myFFT, sourceArrayReal, destArrayImagTmp, destArrayRealTmp, destArrayImagTmp);

    for ( k_ = 0; k_ < myFFT->FFT_SIZE; k_++ )
    {
        destArrayRealTmp[k_] = myFFT->FFT_SIZE_INV * destArrayRealTmp[k_];
        destArrayImagTmp[k_] = -1.0f * myFFT->FFT_SIZE_INV * destArrayImagTmp[k_];
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    regA = vdupq_n_f32(myFFT->FFT_SIZE_INV);
    regB = vdupq_n_f32(-1 * myFFT->FFT_SIZE_INV);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    {
        float max_real;
        if (fabs( destArrayRealTmp[k] ) >=  fabs(destArrayReal[k]))
        {
             max_real = fabs(destArrayRealTmp[k]);
        }
        else
        {
             max_real = fabs(destArrayReal[k]);
        }
        if ((fabs(destArrayRealTmp[k] - destArrayReal[k])/(max_real+1E-8) >= REL_TOL) &&( fabs( destArrayRealTmp[k] - destArrayReal[k] ) > ABS_TOL ))
        {
             log_error("#IFFT# : %f; SIMD: %f\n ", destArrayRealTmp[k], destArrayReal[k]);
             exit(-1);
        }


        float max_image;
        if (fabs( destArrayImagTmp[k]) >= fabs(destArrayImag[k]))
        {
             max_image = fabs(destArrayImagTmp[k]);
        }
        else
        {
             max_image = fabs(destArrayImag[k]);
        }
        if ((fabs(destArrayImagTmp[k] - destArrayImag[k])/(max_image+1E-8) >= REL_TOL) && ( fabs( destArrayImagTmp[k] - destArrayImag[k] ) > ABS_TOL))
        {
            log_error("#IFFT# : %f; SIMD: %f\n ", destArrayImagTmp[k], destArrayImag[k]);
            exit(-1);
        }
    }
...
#ifdef GET_MIPS
    unsigned long long destArray2_circle2 = get_cpu_circle();
    gc->destArray_circle_sum2 += (destArray2_circle2 - destArray2_circle1);
#endif
